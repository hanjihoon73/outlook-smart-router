# Outlook Smart Router (가칭) 크롬 확장앱 기획안 v1.1
...
## 3. 시스템 아키텍처 (Serverless 구조)

본 프로젝트는 별도의 백엔드 데이터베이스 서버를 구축하지 않고(Serverless), 다음의 두 가지 저장소를 활용하여 운영 비용과 복잡도를 최소화합니다.

*   **실제 동작 규칙 저장 (Microsoft Exchange 서버):** 확장앱이 DNF로 쪼갠 규칙들은 Graph API를 통해 사용자의 Outlook 서버에 순정 규칙으로 직접 저장됩니다. 실제 메일 분류 처리는 전적으로 MS 서버가 담당합니다.
*   **원본 규칙 형태 저장 (Chrome 동기화 저장소):** "이 쪼개진 규칙들이 원래는 하나의 그룹이었다"는 메타데이터(UI 상태)는 `chrome.storage.sync`를 활용해 저장합니다. 이를 통해 사용자는 여러 PC를 오가며 크롬을 사용해도 일관된 규칙 설정 화면을 볼 수 있습니다.

## 4. 폴더 구조 및 파일 구성 (Folder Structure)
